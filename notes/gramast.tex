...
% \TODO[inline]{schichten früher einführen - dao -def}
% \TODO[inline]{Schicht nennen? oder einfach nur selbst-modifikation}
...
Eine DAO ist ein relativ neues Konzept, welches durch das Populär-werden von Technologien, wie Bitcoin und Ethereum, erstmals ins Interesse der Fachweld gerückt ist. Diese ist sich über die genaue Definition und die Abgrenzung zu verwanten Konzepten größtenteils noch uneinig.
...
% Alle Akteure besitzten eine Kopie eines gemeinsamen Konsenses. Durch einen Dezentralen Mechanismus wird ein Akteur ausgewählt, welcher den derzeitigen Konsens erweitern kann. Die Erweiterung wird darauf hin von allen Akteuren als Konsens angenommen, falls diese konsistent ist. Hiernach wird nach einiger Zeit der Mechanismus erneut ausgelöst. Damit ein Akteur zum erweitern des Konsenses ausgewählt wird, muss dieser einen Beweis liefern, dass dieser über eine bestimmte Menge an Rechenressourcen verfügt. Die Menge der Rechenressourcen ist natürlich knapp.
...
Jedoch gibt es Argumente dafür, dass die Blockchain das Problem der Byzantinischen Generäle nicht vollständig löst. Die Bitcoin - Blockchain hatte jedoch nicht den Anspruch, das BG Problem vollständig zu lösen. Es löst ledignlich eine schwächere Version, nämlich die der Einigung der Generäle.
...
Der eigentliche Block besteht dabei aus einer Referenz des vorherigen Blocks $(ref)$ zusammen mit dem neuen Konsens ($kons$) und einer $nonce$, die frei wählbar ist. Zudem existiert eine, durch die vorherigen Blöcke bestimmte, Schwierigkeit ($\varepsilon$), so dass die Rechenzeit für den neuen Block bei 10 Minuten bleibt. Akteure, die im Wettbewerb um einen neuen Block stehen (auch ``Miner'' genannt) müssen nun eine nonce finden, damit ein Hashwert des Blockes unter der Schwierigkeit liegt:
...
% \TODO[inline]{paragraph umformulieren}
...
\begin{enumerate}
  ...
  \item Falls die Signatur und Manipulation valide ist, wird der derzeitige Zustand aktualisiert. Die Transaktion wird dann der Menge valider, bekannter Transaktionen hinzugefügt, die vom Miner in den nächsten Block hinzugefügt werden sollen.
...
% \TODO[inline]{letzten satz umformulieren}
...
Der Gewinner war die Bitcoin-basierte dezentrale Crowdfunding Plattform Lighthouse\footnote{https://www.vinumeris.com/lighthouse}. Diese erfüllt die Ansprüche an eine solche Selbst-Modifikation-Schicht nicht wirklich.
...
\textbf{Im folgenden wird ein Vorschlag einer solchen Schicht vorgestellt. Hierfür werden als Erstes die theoretischen Komponenten betrachten, anschließend ihre Anwendung auf die regulären Grammatiken und schließlich ihre Implementation.}


Theoretisch kann ein solcher Mechanismus durch ein \textbf{initialisiertes Transitionssystem $T=(M,I,\tau)$} nach \cite{Glausch} modelliert werden. Hierzu muss eine passende Zustandsmenge definiert werden, einen Initialzustand sowie die Übergangsfunktion, die valide Manipulationen (Transitionen) der DAO beschreibt.

Durch die Dezentralisierung-Schicht wird sichergestellt, dass Transitionen \textbf{immer} von einem Akteur $a$ ausgelöst werden und das dieser Akteur ebenfalls ihr Urheber ist. Damit besitzt eine Transition die Form $(a, O')$. $O'$ beinhaltet dabei die Manipulationen des Akteurs.
...
% \TODO[inline]{ANwendung nicht implementierung}
% \TODO[inline]{drauf bezug nehmen}
...
% \TODO[inline]{paragraph vereinfachen}
...
% \TODO[inline]{Isomorph -> Isomorphismus : die uns den gewünschten Isomorphismus liefert }
...
Sei $S(G) = (T', N', S', P')$ eine erweiterte Grammatik. Sei weiter $O,S'\in N'$ sowie $v,o_1,o_2 \in T'^*$, $o_1\neq 0_2$ und $0\leq i\leq j\leq n$ mit $i,j,k,n \in \mathbb{N}$, dann ist das Erzeugen einer Optionsmenge valide, wenn folgende Bedingungen erfüllt sind:
...
Sei $v_1, v_2 \in T'^*$ sowie $n\in\mathbb{N}$, so lässt sich eine Stimme valide löschen, wenn folgende Bedingungen erfüllt sind:
...
\begin{eqnarray}
  w &=& w_{0 .. i-1}\ w_{i..j}\ w_{j+1 .. n} \\
  b_1, ..., b_6 &\in& \{ w | w = ([h\ n])^*\text{ mit } h \in HASH\text{ und }n\in\mathbb{N} \} \\
