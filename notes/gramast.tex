...
Die Manipulation wird durch ein \textbf{initialisiertes Transitionssysthem $T=(S,I,\tau)$} beschrieben.
...
Ein Zustand ist eine Struktur der Form $(U, w) \in S$
...
\subsubsection*{Transitionsbedingungen}
Um die Validität einer Transaktion zu verifizieren, wird das derzeitige Wort $w$ zusammen mit der Aktualisierung $(a,w')$ betrachtet. Die Transaktion ist valide, falls eines der folgenden Fälle zutrifft:
...
\[ \phi^{-1}(w) \cap \phi^{-1}(w_{0..i-1}\ \alpha[r\oplus []][d]\ w_{j+1..n}) = \varnothing \] 
